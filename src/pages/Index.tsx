      ...
      <section className="relative flex min-h-[70vh] flex-col items-center justify-center bg-gradient-to-b from-primary via-nature-canopy to-nature-deep px-4 text-center text-primary-foreground">
        ...
        <div className="relative z-10 max-w-3xl animate-fade-in-up">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            {zones.length > 0 && (
              <Link to={`/zone/${zones[0].slug}`} className="rounded-lg bg-accent px-6 py-3 font-semibold text-accent-foreground shadow transition hover:opacity-90">
                Start Exploring
              </Link>
            )}
          </div>
        ...
        <div className="container mx-auto px-4">
          ...
          {isLoading ? (
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {zones.map((zone) => (
                <ZoneCard key={zone.id} zone={{ id: zone.slug, name: zone.name, tagline: zone.tagline, icon: zone.icon, description: zone.description, highlights: zone.highlights, facts: zone.facts }} />
              ))}
            </div>
          )}
        </div>
